...
Mat BilinearFilter::update(const Mat& rgb, const Mat& depth, deque<Mat>& previous_frames)
{
  ...
  Mat curr_rgb, curr_depth;
    
  Mat out(depth.size(), depth.type());
  
  for(int t = 0; t < previous_frames.size(); t = t+2) {
    
    curr_rgb = previous_frames[t];
    curr_depth = previous_frames[t+1];
    
    copyMakeBorder(curr_rgb, b_rgb, kernel_size, kernel_size, kernel_size, kernel_size, BORDER_REFLECT);
  
    copyMakeBorder(curr_depth, b_depth, kernel_size, kernel_size, kernel_size, kernel_size, BORDER_REFLECT);
    ...
    for (int i = kernel_size; i < b_rgb.rows-kernel_size; i++) {
      for (int j = kernel_size; j < b_rgb.cols-kernel_size; j++) {
        ...
        for (int k = -kernel_size; k < kernel_size+1; k++) {
          for (int l = -kernel_size; l < kernel_size+1; l++) {
            ...
            if(b_depth.at<unsigned short>(i+k, j+l) == 0)
              continue;
            double w_d = threeDMat.at<double>(kernel_size+k, kernel_size+l, t); //Using new Kernel with now t=0 No prev buffer
        ...
        out.at<unsigned short>(i-kernel_size, j-kernel_size) += result/(double)t_range;
      }
    }
  }
